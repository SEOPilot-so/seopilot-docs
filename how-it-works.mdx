...
Each morning, the next article in your queue is either published automatically or surfaced for your review. In **Autopilot** mode, the article is sent to your site via your configured webhook or API connection — no action needed from you. In **Review-First** mode, you'll receive a notification that a new article is ready. You can read it, make edits, and publish it whenever you're ready, or skip it if the topic no longer feels relevant.
...
- **Structured data (schema markup)** — Article and FAQ schema are included automatically to improve how your content appears in search results.
...
- **Pull API** — Fetch new articles from SEOPilot on your own schedule, on demand.
- **MDX export** — Download articles as `.mdx` files, ideal for Next.js, Astro, or any Markdown-based site.
- **CSV export** — Download articles in bulk as a spreadsheet for manual upload workflows.

## Autopilot vs. Review-First

Not sure which publishing mode is right for you? Here's how they compare:

| Feature | Autopilot | Review-First |
|---|---|---|
| **Publishing speed** | Immediate — articles go live as soon as they're drafted | On your schedule — articles publish when you approve them |
| **Your involvement** | None required after setup | You read and approve each article before it publishes |
...
You can switch between modes at any time in your account settings. Many users start on Review-First to build confidence in the output, then switch to Autopilot once they've seen SEOPilot's style match their brand.
...
<CardGroup cols={2}>
  ...
  <Card title="Publishing & Integrations" icon="newspaper" href="/features/publishing">
    Explore all publishing options in detail — webhook setup, pull API reference, and MDX/CSV export workflows.
  </Card>
</CardGroup>
