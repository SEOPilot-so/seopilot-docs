...
<Steps>
  ...
  <Step title="Add your webhook URL to SEOPilot">
    Go to **Settings → Publishing** and enter `https://yoursite.com/api/seopilot` as the webhook URL.
  </Step>
  <Step title="Deploy and test">
    Deploy your changes, then use the **Send test payload** button in SEOPilot to confirm the handler writes the file and revalidates correctly.
  </Step>
</Steps>
...
The MDX frontmatter that SEOPilot exports (`title`, `slug`, `meta_description`, `keyword`, `published_at`) maps directly to a standard Astro content collection schema — no transformation needed. Define your collection in `src/content/config.ts` and Astro will pick up new files automatically on the next build.
...
Hugo reads Markdown content from `content/posts/`. Point your webhook handler to write each article's `body_mdx` field (Hugo accepts standard Markdown with YAML frontmatter) into `content/posts/${slug}.md`, then trigger a rebuild via your hosting provider's deploy hook.
...
<Steps>
  ...
  <Step title="Save the article body">
    Write `article.body_mdx` (or `article.body_html`) to the appropriate content directory for your framework.
  </Step>
