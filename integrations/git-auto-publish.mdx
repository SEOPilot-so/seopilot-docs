...
SEOPilot POSTs this JSON envelope to your endpoint. The handler in the next step maps `data.article` (and `data.keyword`) to an MDX file; the remaining fields — `delivery_id`, `created_at`, `data.site`, `meta_title`, `internal_links` — are there if you want them. See the [Webhook guide](/integrations/webhook#webhook-payload) for the full field reference.
